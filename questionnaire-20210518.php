...
<html lang="ja">
  ...
  <body>
	...
	echo "&nbsp;<font color=\"#FF8000\">実験&nbsp;</font>", $_SESSION["r_count"] - $execise_no + 1, "/${sti_name}問";
      } else {
	echo "&nbsp;実験に関するアンケート";
      }
      ?>
	...
	$sfile0 = $_SESSION["s_seq"][$_SESSION["r_count"]];
      ...
      if ($rc <= $select_no) { //読影終了でなければ
	print "$toi_explanation"; // 各問の説明を表示
	// X線画像の表示
	print "<img id=\"xray\" src=\"image-xray/${sfile0}\" width=\"{$image_width}\">";
	// タコメータ表示（AIの感度・特異度）
	print "<br><br>";
	//	if ((($ab_ba == 0) && ($rc <= $select_no/2)) | (($ab_ba == 1) && ($rc >= $select_no/2))) {
	if (preg_match("/^pn/", $sfile0) | preg_match("/^nn/", $sfile0)) { //
	  print "<img id=\"taco\" src=\"image/taco-meter90.png\" width=\"100\">";
	} else {
	  print "<img id=\"taco\" src=\"image/taco-meter60.png\" width=\"100\">";
	}
	print "<small>　AIの確信度</small>";
	...
	foreach ($ques_list as [$k, $ques]) {
	 ...
	 <td width="160">まったくそう思わない</td>
	 <td width="160">そうは思わない</td>
	 <td width="160">あまりそうは思わない</td>
	 <td width="160">どちらでもない</td>
	 <td width="160">ややそう思う</td>
	 <td width="160">そう思う</td>
	 <td width="160">非常にそう思う</td>
	</tr>
	 <tr align="center">
...
EOT;
	 ...
	 <tr style="font-size:12px"; align="center">
	 <tr style="font-size:14px"; align="center">
	 <td width="160">まったく見なかった</td>
	 <td width="160">時々見た</td>
	 <td width="160">読影毎に見た．</td>
	</tr>
	 <tr align="center">
     <div class="radio">
	 <td><input type="radio" name="a$n" value="1" required></td>
...
EOT;
	...
	print <<<EOT
        【自由記述】途中で『この選択はよくないかも知れません』というアラートが表示されていた場合，それをどのように解釈しましたか？下記に回答下さい．（句読点をお願いします．改行OK．）<br>もし表示されなかった場合は，空欄のままで結構です．
	<textarea rows="10" cols="80" name="t1"></textarea> <br>
	【自由記述】この実験についてのご意見，感想をお書き下さい．（句読点をお願いします．改行OK．）<br>
	<textarea rows="10" cols="80" name="t2" minlength="8" required></textarea>
EOT;
      }
      ?>
    </div>
    ...
    <p>
      <?php
      if ($_SESSION["r_count"] < $execise_no + $select_no) { // 練習or本読影でテーブル表示
	print <<<EOT
      <table>
	<tr>
	  <th>
...
EOT;
    ...
    <p>
      <input type="submit" value="次ページ" name="submit">
    </p>
  </div>
  </body>
</html>
